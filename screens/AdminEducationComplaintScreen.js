...
export default class AdminEducationComplaint extends React.Component {
  constructor() {
    super();
    this.state = {
  ...
  getRequestedComplaintsList = (requestedState) => {
    var state = this.state.stateRequested.toLowerCase();
    if (state.length > 0) {
      this.requestRef = db
        .collection('all_complaints')
        .where('Complaint_Type', '==', 'Educational Complaint')
